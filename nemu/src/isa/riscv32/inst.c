...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, s->dnpc = (src1 + imm) & 0xFFFFFFFE; R(rd) = s->pc + 4; ftrace_get_addr(s->pc, s->dnpc, BITS(s->isa.inst, 19, 15), rd, imm));
  ...
  //TYPE_J
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, s->dnpc = s->pc + imm; R(rd) = s->pc + 4; ftrace_get_addr(s->pc, s->dnpc, BITS(s->isa.inst, 19, 15), rd, imm));
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R, if(src2 == 0) R(rd) = -1; else if(src1 == 0x80000000 && src2 == 0xffffffff) R(rd) = 0x80000000; else R(rd) = (sword_t)src1 / (sword_t)src2); //printf("src1=%x, src2=%x\n",src1,src2);
  ...
  INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu , R, R(rd) = ((int64_t)(sword_t)src1 * (int64_t)src2) >> 32); 
